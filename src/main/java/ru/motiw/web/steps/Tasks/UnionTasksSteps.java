    ...
    private void checkDisplayCreateAFolderInTheGrid(String folder, boolean useAFilter) {
        if (useAFilter) {
            ...
            $(By.xpath("//b[contains(text(), '" +
                    parseNameFolder(folder)[0] + "')]/ancestor::div[contains(@class,'x-grid-cell')]/child::div[@class=' x-tree-icon x-tree-icon-custom x-tree-icon-leaf bfolder-smart']")).waitUntil(visible, 10000).shouldBe(visible);
        } else
